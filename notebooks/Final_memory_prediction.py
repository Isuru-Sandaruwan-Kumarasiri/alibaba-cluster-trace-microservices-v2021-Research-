...
FE_OUTPUT_ROOT = "s3://research-s20426/tgnn_dataset_v2"   # EDIT to your actual FE output root
...
TARGET_COL_OVERRIDE = None      # e.g. "memory_util_rs"
...
target_candidates = [c for c in node_cols if "memory" in c.lower()]
TARGET_COL = TARGET_COL_OVERRIDE or (target_candidates[0] if target_candidates else None)
assert TARGET_COL, "Couldn't find a memory-related column — set TARGET_COL_OVERRIDE manually."
...
ax.legend()
plt.tight_layout()
fig.savefig("train_test_val_by_RMSE.png", dpi=150, bbox_inches="tight")
...
axes[0].set_xlabel("Actual (scaled memory target)")
...
# save_artifact("predicted_vs_actual.png")
plt.show()
